...
Cus.Lasso <- function(X,Y,Z,sigma.square = estimateVariance(X,Y),
...
update_alpha.lasso<-function(X,Y,Z,alpha.old,sigma.square,theta,maxstep_inner,tol.inner){
        ...
        while (k_inner < maxstep_inner){
                ...
                alpha.iner.new <- stats::optim(alpha.old,likelihood.alpha.theta.lasso,likelihood.alpha.theta.gradient.lasso,Z=Z,theta = theta,delta=delta.est,method = "BFGS")$par
