...
test_that("throw error when X or Z do not have at least 2 columns", {
        ...
        Y <- 1:10
        Z <- matrix(runif(10), 1)
        expect_error(ipreg(X,Y,Z), "X must be a matrix with 2 or more columns")
})
...
test_that("throw error when dimensions of X and Y do not match", {
        ...
        Y <- 1:10
        Z <- matrix(runif(10), nrow = 5)
        expect_error(ipreg(X,Y,Z), paste("number of observations in Y (", length(Y), ") not equal to the number of rows of X (",
...
test_that("throw error when ncol(X) not equal to nrow(Z)", {
        ...
        Y <- 1:5
        Z <- matrix(runif(10), nrow = 5)
        expect_error(ipreg(X,Y,Z), paste("number of rows in Z (", nrow(Z),
...
test_that("throw error when Z matrix contains non-numeric value",{
        ...
        Z[1,1] <- "0.88"
        expect_error(ipreg(X,Y,Z), paste("Z contains non-numeric values"))
})
